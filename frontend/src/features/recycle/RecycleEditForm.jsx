...
// fixed here (use delete + re-record if that needs to change).
function RecycleEditForm({ batch, onSaved, onCancel }) {
  const [vehicleNumber, setVehicleNumber] = useState(batch.vehicle_number);
  const [driverName, setDriverName] = useState(batch.driver_name);
  ...
  async function handleSubmit(e) {
    ...
    setError(null);
    try {
      await apiClient.patch(`/recycle/${batch.id}`, { vehicleNumber, driverName });
      onSaved();
    } catch (err) {
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <p className="rounded-md border border-dashed border-slate-200 px-3.5 py-2.5 text-sm text-slate-500 dark:border-surface-700 dark:text-neutral-400">
            {batch.battery_count} (fixed — delete and re-record to change)
          </p>
        </div>
      </div>
...
export default RecycleEditForm;
